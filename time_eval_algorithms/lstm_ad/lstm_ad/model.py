...
class LSTMAD(nn.Module):
    def __init__(self,
        ...
        self.lstms = nn.LSTM(input_size=input_size, hidden_size=self.hidden_units *
                             self.prediction_length, batch_first=True, num_layers=lstm_layers)
        self.dense = nn.Linear(in_features=self.window_size * self.hidden_units *
                               self.prediction_length, out_features=self.hidden_units * self.prediction_length)
        self.anomaly_scorer = AnomalyScorer()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x, hidden = self.lstms(x)
        x = x.reshape(-1, self.window_size *
                      self.hidden_units * self.prediction_length)
        x = self.dense(x)
        return x
    ...
    def _split_data(self, ts: np.array) -> Tuple[DataLoader, DataLoader]:
        ...
        train_ds = TimeSeries(train_ts, window_length=self.window_size,
                              prediction_length=self.prediction_length, output_dims=self.output_dims)
        valid_ds = TimeSeries(valid_ts, window_length=self.window_size,
                              prediction_length=self.prediction_length, output_dims=self.output_dims)
